...
export default function ResultClient({ result, checkId, inputText = '', scoreSteps = [], tier = 'full', confidenceScore, isLoggedIn = false, userToken }: Props) {
  ...
  async function copyReport() {
    ...
    try {
      ...
      // Fallback for older Android browsers that block clipboard API
    ...
    setCopied(true)
    toast.success(lang === 'tl' ? 'Nakopya ang report!' : 'Report copied!')
        ...
        {tier === 'guest' && (
          <div className="bg-ink rounded-2xl p-5 space-y-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-bold text-white">You've used your free check.</p>
                <p className="text-xs text-white/50 mt-0.5">Sign up free to get 3 checks per month.</p>
                <p className="text-[10px] text-white/30 mt-1">Nagamit mo na ang iyong libreng check. Mag-sign up para makakuha ng 3 checks bawat buwan.</p>
              </div>
            </div>
            <div className="space-y-2">
